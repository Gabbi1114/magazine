      ...
      <main className="pointer-events-none select-none z-10 fixed inset-0">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-6">
            ...
            {/* Image upload */}
            <section className="flex flex-col gap-3">
              <p className="text-white/40 text-xs font-semibold uppercase tracking-widest">Page Images</p>
              {isSpreadView ? (
                <div className="flex flex-col gap-3">
                  <div className="flex gap-3">
                    {leftPage && (
                      <UploadSlot
                        label="Left"
                        imageUrl={pageImages[leftPage.id]?.back}
                        onFileSelected={(file) => openCrop(leftPage.id, "back", file)}
                      />
                    )}
                    {rightPage && (
                      <UploadSlot
                        label="Right"
                        imageUrl={pageImages[rightPage.id]?.front}
                        onFileSelected={(file) => openCrop(rightPage.id, "front", file)}
                      />
                    )}
                  </div>
                  {/* Full editor buttons */}
                  <div className="flex gap-2">
                    {leftPage && (
                      <button
                        className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 hover:text-white border border-indigo-500/25 hover:border-indigo-400/50 text-xs font-medium transition-all"
                ...
                <div className="flex flex-col gap-3">
                  <UploadSlot
                    ...
                    imageUrl={pageImages[pages[0].id]?.front}
                    onFileSelected={(file) => openCrop(pages[0].id, "front", file)}
                  />
                  <button
                    className="flex items-center justify-center gap-1.5 py-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 hover:text-white border border-indigo-500/25 hover:border-indigo-400/50 text-xs font-medium transition-all"
                ...
                <div className="flex flex-col gap-3">
                  <UploadSlot
                    ...
                    imageUrl={pageImages[pages[pages.length - 1].id]?.back}
                    onFileSelected={(file) => openCrop(pages[pages.length - 1].id, "back", file)}
                  />
                  <button
                    className="flex items-center justify-center gap-1.5 py-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 hover:text-white border border-indigo-500/25 hover:border-indigo-400/50 text-xs font-medium transition-all"
                ...
                <p className="text-white/25 text-xs text-center py-6 border border-dashed border-white/10 rounded-xl">
                  Navigate to an interior page to upload images
                </p>
              )}
            </section>
